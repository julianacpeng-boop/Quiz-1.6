...
def criar_pagina_base(tema, perguntas):
    ...
    for idx, pergunta in enumerate(perguntas, start=1):
        ...
        # Alternativas
        # Se "imagens_opcoes" não existir, mantém exatamente o layout antigo.
        imagens_valores = pergunta.get("imagens_opcoes", pergunta.get("imagens"))
        tem_imagens = imagens_valores is not None and any(
            v is not None and str(v).strip()
            for v in imagens_valores
        )

        if tem_imagens:
            # Tamanho automático para continuar cabendo na mesma folha.
            if total <= 3:
                img_size = 58
            elif total == 4:
                img_size = 42
        ...
        row_content_h = max(a_lh, img_size if tem_imagens else 0)
        row_gap = 6 if tem_imagens else 8
        ...
        for j, (letra, alt) in enumerate(
            ...
            if imagens_paths[j] is not None:
