...
def preparar_imagem_pergunta(caminho, largura=300, altura=190):
    """Prepara uma imagem única para ficar ao lado das três alternativas."""
    figura = Image.open(caminho).convert("RGBA")
    resampling = getattr(Image, "Resampling", Image)
    figura.thumbnail((largura - 12, altura - 12), resampling.LANCZOS)
    ...
    x = (largura - figura.width) // 2
    y = (altura - figura.height) // 2
    card.alpha_composite(figura, (x, y))
    return card
...
def criar_pagina_base(tema, perguntas):
    ...
    for idx, pergunta in enumerate(perguntas, start=1):
        ...
        if tem_imagem_unica:
            # Reserva a direita para a imagem que contém as três opções visuais.
            img_card_w = 300 if total <= 3 else 240
            img_card_h = 190 if total <= 3 else 150
            image_card_x = right - img_card_w
            text_x = x_option
            text_right = image_card_x - 22
            img_size = 0
